...
Download YouTube audio with a hardware-button gesture. Play any video in the YouTube app, press **Volume-Up + Volume-Down together** for a quarter second, and the audio is saved to your Music folder — no menus, no copy-paste, no foreground app required.
...
Hold Vol-Up + Vol-Down for 250ms
        |
        v
AccessibilityService detects the combo and fires a trigger
        |
        v
...
3. Plug in an Android phone (API 26+, ARM64) and Run.
...
The accessibility service only reads volume key events. It does not read screen content, intercept other keys, or interact with other apps.

## Usage

1. Play any song or video in the YouTube or YouTube Music app.
...
- **Room** for the download history
- **WorkManager** for retry on transient failures
- **Coroutines** for the foreground service work
...
- **ARM64 only in practice.** Tested on `arm64-v8a` devices; should also work on x86_64 since there are no native binaries, but untested.
...
- [NewPipeExtractor](https://github.com/TeamNewPipe/NewPipeExtractor) for the YouTube stream extraction.
- [yt-dlp](https://github.com/yt-dlp/yt-dlp) — originally bundled, removed once it became clear no Linux yt-dlp build runs on Android's bionic libc.
